...
MEDIANS_NAME = "train_medians.joblib"  # optional, created by training scripts
...
def load_artifacts(preproc_p, labelenc_p, model_p, medians_p=None):
    if not os.path.exists(preproc_p) or not os.path.exists(labelenc_p) or not os.path.exists(model_p):
        raise FileNotFoundError("Required artifact not found. Make sure preprocessor, label encoder and model exist.")
...
if hasattr(preprocessor, "feature_names_in_"):
    ...
    # fallback: try to collect raw columns from transformers
    try:
        feature_names = []
...
# -----------------------
# Utility: convert preprocessor output to dense (for display)
# -----------------------
...
# -----------------------
mode = st.radio("Mode", ["Single prediction (one textbox)", "Batch CSV prediction"], horizontal=True)
